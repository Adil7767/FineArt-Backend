...
class TransactionViewTestCase(APITestCase):
    ...
    def test_user_get_transaction_list(self):
        response = self.client_user.get(reverse('transaction-list'))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_admin_get_transaction_list(self):
        response = self.client_admin.get(reverse('transaction-list'))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 4)
        response_data = response.data['results']
        self.assertEqual(response_data[0]['id'], self.admin.id)
        self.assertEqual(response_data[0]['type'], self.type.id)
        self.assertEqual(response_data[0]['payment_method'], self.payment_method.id)
        self.assertEqual(response_data[0]['description'], 'Test transaction')
        self.assertEqual(response_data[0]['category'], self.category.id)
    ...
    def test_user_create_transaction(self):
        ...
        response = self.client_user.post(reverse('transaction-list'), data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_admin_create_transaction(self):
        ...
        response = self.client_admin.post(reverse('transaction-list'), data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Add_Transaction.objects.count(), 2)
        transaction = Add_Transaction.objects.last()
        self.assertEqual(transaction.user, self.admin)
        self.assertEqual(transaction.type, self.type)
        self.assertEqual(transaction.payment_method, self.payment_method)
        self.assertEqual(transaction.description, 'New transaction')
        self.assertEqual(transaction.category, self.category)
    ...
    def test_admin_update_transaction(self):
        ...
        response = self.client_admin.put(url, data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_user_update_transaction(self):
        url = reverse('transaction-detail', kwargs={'pk': self.transaction.pk})
        data = {
            'type': self.type.id,
            'payment_method': self.payment_method.id,
    ...
    def test_admin_delete_transaction(self):
        ...
        self.assertRaises(Add_Transaction.DoesNotExist, self.transaction.refresh_from_db)

    def test_user_delete_transaction(self):
        url = reverse('transaction-detail', kwargs={'pk': self.transaction.pk})
